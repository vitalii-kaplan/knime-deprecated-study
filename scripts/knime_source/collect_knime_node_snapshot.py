...
def main() -> int:
    ...
    if args.source_root.is_absolute():
        print("error: source_root must be relative to the repository root", file=sys.stderr)
        return 2
    if args.out_dir is not None and args.out_dir.is_absolute():
        print("error: --out-dir must be relative to the repository root", file=sys.stderr)
        return 2
